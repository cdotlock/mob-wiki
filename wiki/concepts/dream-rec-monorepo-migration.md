...
把 `AugustZAD/dream-rec` 整体搬进 `cdotlock/lunaverse-backend` 仓的 `services/dream-rec/`，使用 `git subtree add` 保留完整历史。代码层面 monorepo，运行时仍然独立 container + HTTP 通信。
...
**Branch:** `feat/dream-rec-monorepo` pushed to `cdotlock/lunaverse-backend` → **PR [#4](https://github.com/cdotlock/lunaverse-backend/pull/4) opened 2026-05-24，等同事 review。**
...
- 🟡 [PR #4](https://github.com/cdotlock/lunaverse-backend/pull/4) 等同事 review + merge
- ✅ 但 backend [[concepts/dream-rec-integration-architecture#lunaverse-backend-integration-pr-3-merged-2026-05-24|PR #3]] 三个 hook commit 早已合到 backend `main`，等环境变量翻 true 就生效
...
- **Branch:** https://github.com/cdotlock/lunaverse-backend/tree/feat/dream-rec-monorepo
