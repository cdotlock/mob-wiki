...
tags: [mss, script, interpreter, go, visual-novel, fastapi]
sources: []
created: 2026-04-15
updated: 2026-05-30
---
...
- **spec 不再描述迁移路径**（`a5b0135`）— `MSS-SPEC.md` 整体改写按当前模型描述，旧的 "migration notes" 删干净，新读者不需要先理解历史。详见 [`MSS-SPEC.md`](https://github.com/cdotlock/moonshort-script/blob/main/MSS-SPEC.md) + [`README.md`](https://github.com/cdotlock/moonshort-script/blob/main/README.md) 同步刷新。
...
- **FastAPI HTTP wrapper**（`e133bf4` + `d42570c` 补 validate/fix/compile-dir + `cdbe49ce` 写完整 FASTAPI.md）— 新增 `cmd/server` (FastAPI Python wrapper) 暴露 CLI 全功能为 HTTP endpoint，LLM 可自助调用：`POST /compile` / `POST /validate` / `POST /fix` / `POST /compile-dir` / `GET /` 根列表。已部署 stable public URL（Mob Sandbox Daytona），详见 [`FASTAPI.md`](https://github.com/cdotlock/moonshort-script/blob/main/FASTAPI.md)。本仓 Go binary 仍可直跑，FastAPI 是给跨语言客户端 + LLM agent 用的。
