...
| AST | `internal/ast/` | 定义 25+ 种 AST 节点类型。所有 body 级节点内嵌 `ConcurrentFlag` 标记 `&` 并发前缀。条件为**完全结构化 AST**：`Condition` 是 interface，具体类型包括 `ChoiceCondition` / `FlagCondition` / `InfluenceCondition` / `ComparisonCondition` / `CompoundCondition`。终结标记 `EndingNode` 挂在 `Episode.Ending` 字段（与 `Episode.Gate` 互斥）。`SignalNode` 带 `Kind` 字段（`mark` / `achievement`）。`AchievementNode` 声明成就（`Episode.Achievements`，字段对齐 story-achievement-generator） |
...
| Validator | `internal/validator/` | 对 AST 做语义校验：每集必须有 `@gate` 或 `@ending`（二者互斥）、`@ending` 类型白名单（complete/to_be_continued/bad_ending）、`@signal` kind 白名单（mark/achievement）、`@achievement` 字段完整性 + rarity 白名单（uncommon/rare/epic/legendary，禁 common）+ 集内 ID 唯一、brave 选项必须有 check + @on success/fail、safe 选项不能有 check、`@goto` 必须有匹配的 `@label`、选项 ID 不能重复、position/transition/bubble_type/option_mode 白名单校验、条件 AST 结构校验（op 白名单、operand 非空） |
...
**Signal kind 二分 + 成就声明**：`@signal` 必须指定 `kind`——`mark`（持久布尔标记，可被 `@if (NAME)` 查询）或 `achievement`（触发已声明的成就，不作为 flag 存储）。`@achievement <id> { name / rarity / description / when }` 块声明成就，字段结构镜像 [cdotlock/story-achievement-generator](https://github.com/cdotlock/story-achievement-generator) skill 的输出——后者产出成就表，前者作为落点。两种触发路径互补：声明式（引擎监听 marks，自动求值 `when`）和命令式（`@signal achievement <id>` 直接触发）。JSON 顶层 `achievements` 数组恒为数组（无成就时 `[]`）。
