...
| AST | `internal/ast/` | 定义 25+ 种 AST 节点类型。所有 body 级节点内嵌 `ConcurrentFlag` 标记 `&` 并发前缀。条件为**完全结构化 AST**：`Condition` 是 interface，具体类型包括 `ChoiceCondition` / `FlagCondition` / `InfluenceCondition` / `ComparisonCondition` / `CompoundCondition` / `CheckCondition` / `RatingCondition`（后两种 v2.4 新增的 context-local 类型）。终结标记 `EndingNode` 挂在 `Episode.Ending` 字段（与 `Episode.Gate` 互斥）。`SignalNode` 带 `Kind` 字段（v2.4 只认 `mark`，字段保留用于未来扩展）。`AchievementNode` 声明成就（`Episode.Achievements`，v2.4 字段只含 `name` / `rarity` / `description`——移除 `when`）；`AchievementTriggerNode` 是独立的 body step（裸 `@achievement <id>`）。`MinigameNode` 有 `Description` 必填字段和 `Body []Node`（替代 v2.3 的 `OnResult` map）。`OptionNode` 只有 `Body []Node`（替代 v2.3 的 `OnSuccess` / `OnFail`）。`CgShowNode` 新增 `Duration` 和 `Content` 必填字段 |
| Parser | `internal/parser/` | 递归下降解析器，将 token 流组装为 AST。关键设计：通过排除法识别角色指令——`@`/`&` 后的 IDENT 如果不是已知关键字，就视为角色名。条件解析为结构化 AST，采用 `||` < `&&` < primary 优先级的递归下降表达式解析器。支持 `@else @if` 链式条件、嵌套括号、递归深度限制（50 层）。v2.4 后 `@on` 直接报错并给出迁移提示；`@achievement <id>` 按是否跟 `{` 区分声明 vs 触发；`@cg` 和 `@minigame` 解析时强制读取必填新字段 |
| Validator | `internal/validator/` | 对 AST 做语义校验：每集必须有 `@gate` 或 `@ending`（二者互斥）、`@ending` 类型白名单（complete/to_be_continued/bad_ending）、`@signal` kind 白名单（当前仅 mark）、`@achievement` 字段完整性 + rarity 白名单（uncommon/rare/epic/legendary，禁 common）+ 集内 ID 唯一、brave 选项必须有 check（v2.4 不再检查 success/fail 都填满，授权作者省略 else）、safe 选项不能有 check、`@goto` 必须有匹配的 `@label`、选项 ID 不能重复、position/transition/bubble_type/option_mode 白名单校验、条件 AST 结构校验（op 白名单、operand 非空、check.result 白名单、rating.grade 非空）、CG duration 白名单（low/medium/high）+ content 非空、minigame description 非空 |
...
**Signal 单 kind + 成就声明/触发拆分（v2.4）**：`@signal <kind> <event>` 语法保留 kind 词元，但当前只实现 `mark`——持久布尔标记，可被 `@if (NAME)` 查询。成就触发**不**再是 signal kind——改走独立的 `@achievement <id>` 裸指令。声明块 `@achievement <id> { name / rarity / description }`（v2.4 **移除 `when` 字段**）只携带元数据，字段结构镜像 [cdotlock/story-achievement-generator](https://github.com/cdotlock/story-achievement-generator) skill 的输出。触发时机完全由脚本控制——剧作者把 `@if (...) { @achievement X }` 放在想要触发的剧情点。这大幅简化了引擎：不再需要 mark watcher + 自动 when 求值，只需被动响应 `{"type":"achievement","id":X}` step。JSON 顶层 `achievements` 数组恒为数组（无成就时 `[]`）。

**CG 视频字段（v2.4 新增）**：`@cg show` 块必须在 body 节点之前声明 `duration:` 和 `content:` 两个字段。`duration` 是档位（`low` / `medium` / `high`），`content` 是英文连续叙述（镜头走向 + 情节）。这两个字段供 agent-forge 视频管线消费——MSS 是流程控制 + 内容，剧本必须带镜头描述，否则下游没法生成 CG 视频。

**Context-local 条件（v2.4 新增）**：`check.success` / `check.fail` 只在 brave option 体内合法，`rating.<grade>` 只在 `@minigame` 体内合法。这两种条件替代了 v2.3 及更早的 `@on success`/`@on fail` 和 `@on S/A B` 块——现在统一走标准 `@if { } @else { }` 语法树。编译器**不**做作用域静态校验（作用域错放运行时恒为 false，视为作者的剧情 bug），JSON 输出为 `{"type":"check","result":"..."}` 和 `{"type":"rating","grade":"..."}`。

**Minigame description（v2.4 新增）**：`@minigame <id> <ATTR> "<description>" { }` 第三位参数强制要求英文描述。小游戏是预制库选取（game_id 查映射表），description 只是剧情粘合——给下游视觉/视频管线用，一句话即可。
...
项目内置了 `skills/mss-scriptwriting/` 目录，包含完整的 Agent Skill，教 LLM 如何正确生成 MSS 脚本。覆盖完整语法规则、27 条常见错误（v2.4 扩充）、`@ending` 终结指令、结构化条件语法（含 context-local `check`/`rating`）、CG `duration` + `content`、minigame description、Remix 差异点、节奏指导、fixer 自动修复能力。Dramatizer 和 Remix Executor 的 LLM 在生成脚本时应加载此 Skill。

## 引擎集成

运行时消费 MSS JSON 的引擎（前端播放器 / 后端数据管线）参考 [`docs/ENGINE-INTEGRATION.md`](https://github.com/cdotlock/moonshort-script/blob/main/docs/ENGINE-INTEGRATION.md) —— 覆盖 signal kind 向前兼容、独立的 achievement step 消费、context-local check/rating 求值、CG duration/content 字段用途、以及 v2.3 → v2.4 迁移 checklist。
